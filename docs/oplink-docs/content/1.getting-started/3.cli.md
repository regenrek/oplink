...
- Commands inherit your shell environment. Set tokens like `DISCORD_BOT_TOKEN` before starting the server, or configure env in your IDE’s MCP entry.
- When you point the server at a `.mcp-workflows/servers.json` that contains `${VAR}` placeholders, startup will fail if those vars aren’t set.
