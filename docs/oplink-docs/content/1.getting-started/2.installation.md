...
- Path A — Quick Start (no init)
  - Best for: trying Oplink fast; no custom workflows; no external OAuth tools yet.
  - How: add an MCP server in your IDE with `--preset thinking`.
  - Command (all IDEs shown below use this under the hood):
    - `npx oplink@latest server --preset thinking`

- Path B — Project Setup (init)
  - Best for: versioned workflows in repo; external servers (Linear/Discord); team sharing.
  - How (one-time):
...
```json
{
  "mcpServers": {
    "oplink": {
      "command": "npx",
      "args": ["oplink@latest", "server", "--preset", "thinking"]
    }
  }
}
```
...
```bash
codex mcp add oplink -- npx -y oplink@latest server --preset thinking
# or
...
```bash
claude mcp add oplink -- npx -y oplink@latest server --preset thinking
# or
...
```bash
claude mcp add-json oplink '{"type":"stdio","command":"npx","args":["oplink@latest","server","--preset","thinking"]}'
```
...
npx oplink@latest add coding
npx oplink@latest add examples
npx oplink@latest add github
npx oplink@latest add template-tools

# From a remote (giget) source
